...
final class HTTPRequest {
	...
	var path = ""
	var headerStore = Dictionary<HTTPRequestHeader.Name, [UInt8]>()
...
final class HTTPResponse: HeaderListener {
...
//				()
	...
	func connect(host hst: String, port: UInt16, ssl: Bool, timeoutSeconds: Double, callback: @escaping (Bool) -> ()) {
		self.host = hst
		self.ssl = ssl
	...
	func createRequest() -> HTTPRequest {
		return HTTPRequest(connection: net)
	}
			...
			if let frame = f {
				...
				case .data:
					if let payload = frame.payload, frame.length > 0 {
						response.bodyBytes.append(contentsOf: payload)
					}
					streamOpen = (frame.flags & flagEndStream) == 0
					if !streamOpen {
						callback(response, nil)
	...
	func sendRequest(_ request: HTTPRequest, callback: @escaping (HTTPResponse?, String?) -> ()) {
		...
		do {
			...
			for (name, value) in request.headerStore {
				...
				let v = UTF8Encoding.decode(string: String(validatingUTF8: value) ?? "")
		...
		writeHTTP2Frame(frame) { [weak self]
			b in
			guard b else {
